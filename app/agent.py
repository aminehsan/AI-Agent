...
1. Create a small ordered plan before using a filesystem tool. If unfinished work already exists, inspect its current state and continue or revise it.
2. Start exactly the next pending step.
3. Execute exactly one filesystem tool for that step. Pass the current step_id and state a precise purpose and expected_result.
4. Inspect the complete tool result, then immediately review the step. Complete it only with concrete evidence; otherwise retry, fail, or revise the unfinished plan.
5. Finish the plan only after its goal is achieved. If completion is impossible, review any pending result and block the plan with a clear reason.
