...
def _write(request: FilesystemRequest) -> CommandSpec:
    ...
    if request.content_format == "base64":
        command = f"mkdir -p {parent} && base64 -D > {path}"
    else:
...
def _name_search(request: FilesystemRequest) -> str:
    ...
    if request.recursive:
        source = f"find {_quote(request.path)} -mindepth 1 -print"
    else:
        source = f"find {_quote(request.path)} -mindepth 1 -prune -print"
    return f"{source} | grep {_grep_flags(request)} {_quote(request.query)}"
